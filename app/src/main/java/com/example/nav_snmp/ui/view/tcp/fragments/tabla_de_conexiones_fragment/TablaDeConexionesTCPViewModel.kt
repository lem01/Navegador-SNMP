...
class TablaDeConexionesTCPViewModel(
    ...
    private val _tablaDeConexionesModel = MutableLiveData<List<TablaDeConexionesModel>>()
    val tablaDeConexionesModel: MutableLiveData<List<TablaDeConexionesModel>>
        get() = _tablaDeConexionesModel
        ...
        when (host.versionSNMP) {
            VersionSnmp.V1.name -> {
                ...
                */
                ...
                _tablaDeConexionesModel.value = unirEnUnaSolaLista(
                    estadoActual,
                    direccionIpLocal,
                    puertoLocal,
                    direccionIpRemota,
    ...
    ): List<TablaDeConexionesModel> {
        val list = mutableListOf<TablaDeConexionesModel>()
        for (i in estadoActual.indices) {
            list.add(
                TablaDeConexionesModel(
                    estadoActual[i],
                    direccionIpLocal[i],
                    puertoLocal[i],
                    direccionIpRemota[i],
