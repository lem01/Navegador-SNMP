package com.example.nav_snmp.ui.view.tcp
...
class TcpActivity : AppCompatActivity() {
    private lateinit var binding: ActivityTcpBinding
    private lateinit var navController: NavController

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        binding = ActivityTcpBinding.inflate(layoutInflater)
...
//            onBackPressedDispatcher.onBackPressed()
        ...
        binding.tabLayout.addOnTabSelectedListener(object : TabLayout.OnTabSelectedListener {
            override fun onTabSelected(tab: TabLayout.Tab) {
                when (tab.position) {
                    0 -> navController.navigate(R.id.datosTcpFragment)
                    1 -> navController.navigate(R.id.tablaDeConexionesTCPFragment)
                }
            }
