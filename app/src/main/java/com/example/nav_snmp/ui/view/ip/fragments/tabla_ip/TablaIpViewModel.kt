...
class TablaIpViewModel(
    private val repository: HostRepository,
    private val context: Context
) : ViewModel() {
    private val TAG = "TablaDeConexionesViewModel"

    private val _tablaIpModel = MutableLiveData<List<TablaIpModel>>()
    val tablaIpModel: MutableLiveData<List<TablaIpModel>>
        get() = _tablaIpModel
    ...
    private suspend fun operacionesSnmp(host: HostModel) {
        ...
        when (host.versionSNMP) {
            ...
            VersionSnmp.V1.name -> {
                ...
                var direccionIp = snmpManagerV1.walk(
                    host,
                    CommonOids.IP.IP_TABLE.ipAdEntAddr,
                    context,
                    false
                )
                var mascaraDered = snmpManagerV1.walk(
                    host,
                    CommonOids.IP.IP_TABLE.ipAdEntNetMask,
                    context,
                    false
                )
                var direccionBroadcast = snmpManagerV1.walk(
                    host,
                    CommonOids.IP.IP_TABLE.ipAdEntBcastAddr,
                    context,
                    false
                )
                var tamanioMaximoDeReensamblaje = snmpManagerV1.walk(
                    host,
                    CommonOids.IP.IP_TABLE.ipAdEntReasmMaxSize,
                    context,
                    false
                )

                _tablaIpModel.value = unirEnUnaSolaLista(
                    direccionIp,
                    mascaraDered,
                    direccionBroadcast,
                    tamanioMaximoDeReensamblaje
                )
            }
    ...
    private fun unirEnUnaSolaLista(
        direccionIp: List<String>,
        mascaraDered: List<String>,
        direccionBroadcast: List<String>,
        tamanioMaximoDeReensamblaje: List<String>
    ): List<TablaIpModel> {
        val list = mutableListOf<TablaIpModel>()
        for (i in direccionIp.indices) {
            list.add(
                TablaIpModel(
                    direccionIp[i],
                    mascaraDered[i],
                    direccionBroadcast[i],
                    tamanioMaximoDeReensamblaje[i],
                )
            )
        }
        return list
...
class TablaIpViewModelFactory(
    private val repository: HostRepository,
    private val context: Context
) :
    ViewModelProvider.Factory {
    override fun <T : ViewModel> create(modelClass: Class<T>): T {
        if (modelClass.isAssignableFrom(TablaIpViewModel::class.java)) {
            @Suppress("UNCHECKED_CAST")
            return TablaIpViewModel(repository, context) as T
        }
